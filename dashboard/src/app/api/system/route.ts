  ...
  try {
    const data = await cached("system:health", 10_000, async () => {
      const [upTargets, chInserted, rpBrokers, ch01Health, ch02Health, rpBrokersAdmin, rpTopics, rpClusterHealth] = await Promise.allSettled([
        ...
        rpGet<{ topic: string; partition_count: number; replication_factor: number }[]>("/v1/topics"),
        rpGet<{ is_healthy: boolean; controller_id: number }>("/v1/cluster/health"),
      ]);
      ...
      const topicsData = rpTopics.status === "fulfilled" ? rpTopics.value : null;
      const clusterData = rpClusterHealth.status === "fulfilled" ? rpClusterHealth.value : null;

      const totalPartitions = topicsData
        ? topicsData.reduce((sum: number, t: { partition_count: number }) => sum + t.partition_count, 0)
        : null;
      const topicNames = topicsData
        ? topicsData.map((t: { topic: string }) => t.topic).filter((n: string) => !n.startsWith("_"))
        : null;
